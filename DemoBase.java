...
public class DemoBase {
    ...
    public PSlider pSlider = new PSlider(
        new Slider(100.0, 25.0, 25.0, 25.0, false, true, 200.0, 300.0, .000001, 1.9, Values.P, Values.timerSpeed));

    public ISlider iSlider = new ISlider(
        new Slider(100.0, 55.0, 25.0, 25.0, false, true, 200.0, 300.0, 0, 100, Values.I, Values.timerSpeed));

    public DSlider dSlider = new DSlider(
        new Slider(100.0, 80.0, 25.0, 25.0, false, true, 200.0, 300.0, -.01, .01, Values.D, Values.timerSpeed));

    public Sprite sprite = new Sprite(
        new Shape(100.0, 100.0, 50.0, 50.0, Values.P, Values.I, Values.D, "oval", Values.timerSpeed));
    ...
    public void run(Graphics g, Mouse m) {
        ...
        g.drawString("P: " + String.format("%.5f", Values.P),
                     (int) ((Values.displayW / 2) + (int) Values.displayW / 5.0 + 5), 30);
        g.drawString("I: " + String.format("%.5f", Values.I),
                     (int) ((Values.displayW / 2) + (int) Values.displayW / 5.0 + 5), 60);
        g.drawString("D: " + String.format("%.5f", Values.D),
                     (int) ((Values.displayW / 2) + (int) Values.displayW / 5.0 + 5), 85);
    }
}